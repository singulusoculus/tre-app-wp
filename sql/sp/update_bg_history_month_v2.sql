...
IF @existscheck IS NULL THEN
    ...
    , round(count(bgg_id) * 16 / MaxList.max_list_count, 3) AS pop_score
    , round(avg(CASE WHEN item_rank <= 100 THEN (100 - item_rank + 1) ELSE 0 END), 3) + round(count(bgg_id) * 16 / MaxList.max_list_count, 3) AS total_score
